...
import SideBar from "../Transport/SideBar";
  ...
  useEffect(() => {
    Axios.get(
      process.env.REACT_APP_BACKEND_URL +
        "/analytics/find/transport?token=" +
        userObj.token
    )
      .then((res) => {
        setShowLoader(false);
                  ...
                  >
                    ...
                    <div className="w-100" style={{ marginLeft: 15 }}>
                      <h2 className="mb-0 quicksand-font">
                        Analytics for transport price per kilometer
                      </h2>
                      <p className="text-gray">
                        The diagram below show how the trasportation price has
                        changed in this month.
                      </p>
                    </div>
                  </div>
